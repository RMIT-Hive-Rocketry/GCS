...
- Lab testing any device that requires external commuincation (GSE or AV)
...
1. Support comprehensive, verbose and useful debugging and post-mortem analysis.

The GCS is meant to not only control everything, but also listen to everything that is going on in the Hive network. So make sure you log EVERYTHING that is useful, and log it at the appropriate level. It currently logs every single byte that goes in and out of the system, the time it happened and associated state transitions. Among other things. Have a read through `logs/`

At the end of the day, if there's a problem with anything you should be able to go through the logs and figure it out. There is no point testing something if you can't analyse the results.
...
| **SUCESS** | Same as info, but highlighted for important pass/fail events. | `GPS fix aquired` | **Low** |
...
| **CRITICAL** | Total system failure. Some system cannot continue and something is very broken. Sometimes indicates data corruption or confirmation that the rocket has actually just exploded | `FC HAS DECIDED TO STOP BROADCASTING` | **Minimal/Zero** |
...
If I see a warning during the launch director's countdown, I want to be able to make up my mind for a abort call ASAP. Maybe it's time we make a GCS playbook for common warnings?
...
This was a change that was made late in the 2025 development cycle to better inform an operator what is actually going on. Because under the hood, we determine if AV has lost signal by checking if the AV sequence lock class has exceeded it's timeout threshold. Although the first warning is programically explicit and better relates to the actual implimentation, the latter makes sense to **everyone else**. People will use this system who have no idea how it works. You're better off to make the warnings super obvious to reduce the cognitive load of the user and erase any possible confusion so they can make calls as quick as possible.
...
3. Reliabllity and consistiency

Avoid flakeyness and race conditions like everything depends on it. Because it does.
Test the hell out of this system prior to every propulsion test and launch. If something doesn't add up, or something is annoying you because it's flakey or not consistent, hunt it down and squash it. We use a 35kv ingnitor that will kill someone if it is ingited at the wrong time. We have an entire pressurised gas system that will also kill someone if it is operated incorrectly. Although most of those issues are protected by safe engineering practices and rigourous operational procedure, the last thing you want is a GCS with unreliable control behaviour.

Also, don't be too scared to make changes to the system that impact this behaviour, but just make sure to test it!

4. Avoid all possible sources of human error

> If your system requires human perfection, your system sucks ass

This whole system is on a computer, automate applicable human error out the equation.

The number one cause of issues with most engineering tasks is human error. If you expect someone to follow a long list of procedures, careful instructions and rules they will become increasingly more likely to introduce error.
...
Also, try and break your system (during runtime testing) in every way. Kill processes, unplug USBs, open prime95. See what happens and protect against it!
