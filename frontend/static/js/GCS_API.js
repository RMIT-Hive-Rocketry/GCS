...
function API_OnMessage(event) {
    ...
    try {
        ...
        if (apiData.id == 3 || apiData.id == 4) {
            ...
            if (typeof updateNetworkDiagnostics2 === "function") {
...
function processDataForDisplay(apiData, apiId) {
    ...
    if (apiId === 50) {
        ...
        // All HTML rendering is handled by updateNetworkDiagnostics2() in GCS_Graphs.js.
        Object.keys(apiData).forEach(device => {
            ...
            // Change radio state to red if ping is -1 (green otherwise)
            if (device === "Vulcan ESP32") {
                processedData.state.gse = {
                    radio: (apiData[device].ping === -1) ? 3 : 1
                };
            }
        });
    }
    ...
    if (apiData?.meta) {
        ...
        if ([3, 4, 5].includes(apiId)) {
            ...
            // Make sure to update the radio state to green or red here, too
            const ping = document.querySelector('input[data-key="Vulcan ESP32.ping"]').value;
            processedData.state.gse = {
                radio: ((ping != null) && (ping === "-1")) ? 3 : 1
            };
        }
    }
