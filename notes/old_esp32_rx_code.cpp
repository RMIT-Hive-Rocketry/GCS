...
static void recv_prase(char *p_msg)
{
    ...
    if (p_start && (1 == sscanf(p_start, "RX \"%s\"", &data)))
    {
        ...
        if (ID == 0x4)
        {
            ...
            float low_accel_x = (int16_t)(((uint16_t)out[2] << 8) | (uint16_t)out[3]) / 2048.0f;
            float low_accel_y = (int16_t)(((uint16_t)out[4] << 8) | (uint16_t)out[5]) / 2048.0f;
            float low_accel_z = (int16_t)(((uint16_t)out[6] << 8) | (uint16_t)out[7]) / -2048.0f;
            float high_accel_x = (int16_t)(((uint16_t)out[8] << 8) | (uint16_t)out[9]) / -1024.0f;
            float high_accel_y = (int16_t)(((uint16_t)out[10] << 8) | (uint16_t)out[11]) / -1024.0f;
            float high_accel_z = (int16_t)(((uint16_t)out[12] << 8) | (uint16_t)out[13]) / 1024.0f;

            float gyro_x = (int16_t)(((uint16_t)out[14] << 8) | (uint16_t)out[15]) * 0.00875f;
            float gyro_y = (int16_t)(((uint16_t)out[16] << 8) | (uint16_t)out[17]) * 0.00875f;
            float gyro_z = (int16_t)(((uint16_t)out[18] << 8) | (uint16_t)out[19]) * 0.00875f;
        ...
        else if (ID == 0x6)
        {
            ...
            uint16_t pl_accel_x_raw = (int16_t)(((uint16_t)out[3] << 8) | (uint16_t)out[2]);
            float pl_accel_x = pl_accel_x_raw & 0x1FFF; // Mask first 3 bits

            uint16_t pl_accel_y_raw = (int16_t)(((uint16_t)out[5] << 8) | (uint16_t)out[4]);
            float pl_accel_y = pl_accel_y_raw & 0x1FFF; // Mask first 3 bits

            uint16_t pl_accel_z_raw = (int16_t)(((uint16_t)out[7] << 8) | (uint16_t)out[6]);
            float pl_accel_z = pl_accel_z_raw & 0x1FFF; // Mask first 3 bits

            float pl_0_temp = (int16_t)(((uint16_t)out[8] << 8) | (uint16_t)out[9]);
            float pl_0_humidity = (int16_t)(((uint16_t)out[10] << 8) | (uint16_t)out[11]);
            float pl_0_pressure = (int16_t)(((uint16_t)out[12] << 8) | (uint16_t)out[13]);

            float pl_1_temp = (int16_t)(((uint16_t)out[14] << 8) | (uint16_t)out[15]);
            float pl_1_humidity = (int16_t)(((uint16_t)out[16] << 8) | (uint16_t)out[17]);
            float pl_1_pressure = (int16_t)(((uint16_t)out[18] << 8) | (uint16_t)out[19]);
...
void setup(void)
{
    ...
    if (at_send_check_response("+AT: OK", 100, "AT\r\n"))
    {
        ...
        at_send_check_response("+TEST: RFCFG", 1000, "AT+TEST=RFCFG,915,SF9,500,12,16,14,OFF,OFF,OFF\r\n");

        // Set E5 baud rate and update arduino serial
        // Uncomment this section and reset the E5 (power cycle or reset command) to take effect
        // at_send_check_response("+UART=BR, 230400", 1000, "AT+UART=BR, 230400\r\n");
        // at_send_check_response("+RESET: OK, 9600", 1000, "AT+RESET\r\n");
        // delay(2000);
