...
def get_command() -> List[str]:
    ...
    script_path = os.path.join(project_root, "backend", "pendant_emulator.py")

    if system == "Darwin":
        # Use single quotes to handle spaces in paths
        shell_cmd = (
            f"export PYTHONPATH='{project_root}'; python3 '{script_path}' -u"
        )
...
def start_pendant_emulator(logger: logging.Logger) -> None:
    ...
    try:
        ...
        with open(
            os.path.join(os.path.sep, "tmp", "GCS_CONFIG_LOCATION.txt"), "w"
        ) as f:
            f.write(
                os.path.join(
                    os.path.abspath(os.getcwd()), "config", "config.ini"
                )
            )
        logger.debug(
        ...
        pendant_process = PendantEmulatorSubprocess(
            PENDANT_EMULATOR_COMMAND, name=SERVICE_NAME, env=env
        )
