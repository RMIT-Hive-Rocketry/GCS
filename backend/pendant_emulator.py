...
def print_information():
    ...
    def _handle_truth_value(
        ...
        if list(TRUTH_VALUES.values()).count(True) > 1:
            slogger.error("Invalid system select state when printing information")
...
def handle_controller_events():
    ...
    while not stop_event.is_set():
        if N2O_FILLING:
            total_secs = (
                time.monotonic() - N2O_FILL_START_TIME + sum(TOTAL_N2O_FILL_TIMES)
            )
                ...
                match event.type:
                    ...
                    case pygame.JOYDEVICEADDED:
                        if not firstAddedEvent:
                            ...
                            controller_offline = (
                                True  # This is default behaviour for F710 controller
                            )
