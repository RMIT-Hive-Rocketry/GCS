...
UartE5Interface::~UartE5Interface() {
...
bool UartE5Interface::initialize() {
  ...
  if (uart_fd_ < 0) {
    ...
    slogger::warning(
        "The UART E5 interface requires a radio to be connected. Perhaps you "
        "mean to use the TEST_UART_E5 interface? (`--interface TEST_UART_E5`)");
...
void UartE5Interface::configure_uart() {
  struct termios tty;
...
std::vector<uint8_t> UartE5Interface::read_with_timeout(int timeout_ms) {
...
ssize_t UartE5Interface::write_serial(const std::vector<uint8_t>& data) {
...
ssize_t UartE5Interface::read_data(std::vector<uint8_t>& buffer) {
  std::lock_guard<std::recursive_mutex> lock(io_mutex_);
  if (uart_fd_ < 0) {
...
std::vector<uint8_t> UartE5Interface::hex_string_to_bytes(
...
/// @return
ssize_t UartE5Interface::write_data(const std::vector<uint8_t>& data) {
  std::lock_guard<std::recursive_mutex> lock(io_mutex_);
  if (uart_fd_ < 0) {
