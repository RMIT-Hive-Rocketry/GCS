...
class GSE_TO_GCS_DATA_2 {
 public:
  ...
  GSE_TO_GCS_DATA_2(const uint8_t *DATA) {
    ...
    internal_temp_ = std::bit_cast<float>(parser.extract_bits(32));
    wind_speed_ = std::bit_cast<float>(parser.extract_bits(32));
    gas_bottle_weight_1_ = static_cast<unsigned int>(parser.extract_bits(16));
    gas_bottle_weight_2_ = static_cast<unsigned int>(parser.extract_bits(16));
    analog_voltage_input_1_ = std::bit_cast<float>(parser.extract_bits(32));
    analog_voltage_input_2_ = std::bit_cast<float>(parser.extract_bits(32));
    additional_current_input_1_ = std::bit_cast<float>(parser.extract_bits(32));
    additional_current_input_2_ = std::bit_cast<float>(parser.extract_bits(32));
