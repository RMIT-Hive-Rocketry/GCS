...
class Packet(ABC):
    ...
    def _setup_logging(cls) -> None:
        ...
        cls._CSV_FILES_WITH_HEADERS = {
            "GCS_TO_AV_STATE_CMD": ["not used"],
            "GCS_TO_GSE_STATE_CMD": ["not used"],
            "AV_TO_GCS_DATA_1": [
                "rssi",
                "snr",
    ...
    def __init__(self, packet_id, serial_data):
        ...
        )  # Automatically set for subclasses
    ...
    def _log_to_csv(self, PROTO_DATA: list[str]):
        ...
        if (
            self._packet_name
            not in self.__class__._CSV_FILES_WITH_HEADERS.keys()
        ):
            raise Exception(
        ...
        with open(
            os.path.join(self.__class__._session_log_folder, file_name), "a"
        ) as f:
            writer = csv.writer(f)
            timestamp = (
                datetime.datetime.now()
                - self.__class__._VIEWER_STARTUP_TIMESTAMP
            )
...
class AVPacket(Packet):
    def __init__(self, ID, serial_data):
        ...
        self._last_information_display_time = (
            time.monotonic()
        )  # Fake minimum starting time
        AVPacket._last_flight_state: FlightState_pb.FlightState = None
        AVPacket._last_state_flags = {
            "dual_board_connectivity_state_flag": None,
        ...
        AVPacket._last_test_details = {
            "apogee_primary_test_complete": None,
    ...
    def _process_state_flags(
        self, PROTO_DATA: AVStateFlags_pb.AVStateFlags
    ) -> None:
...
class AV_TO_GCS_DATA_1(AVPacket):
    ...
    def _process_alt_velocty(
        ...
        if self._max_velocity_updated:
            ...
            slogger.info(f"New max velocity: {MACH_NUMBER:<3.3f} Mach")
            self._max_velocity_updated = False
    ...
    def process(self, PROTO_DATA: AV_TO_GCS_DATA_1_pb.AV_TO_GCS_DATA_1) -> None:
        ...
        if PROTO_DATA.broadcast_flag and self._last_broadcast_value == False:
            ...
            PROTO_DATA.broadcast_flag == False
            and self._last_broadcast_value == True
        ):
...
class GSEPacket(Packet):
    ...
    @classmethod
    def _process_gse_state_flags(
        cls, GSEStateFlags: GSEStateFlags_pb.GSEStateFlags
    ) -> None:
        # Info level for 0 -> 1 and warning for 1 -> 0
        # Same numbers in proto file for listFields index
    ...
    def _process_gse_errors(cls, PROTO_GSE_ERRR_DATA: GSEErrors_pb.GSEErrors):
        ...
        for (
            error_flag_name,
            error_flag_value,
        ) in PROTO_GSE_ERRR_DATA.ListFields():
...
class GSE_TO_GCS_DATA_1(GSEPacket):
    ...
    def _process_trans_therm(
        ...
        TRANSDUCER_VALUE_ERROR = [
            (
                PROTO_DATA.transducer_1,
                PROTO_DATA.error_flags.transducer_1_error,
            ),
            (
                PROTO_DATA.transducer_2,
                PROTO_DATA.error_flags.transducer_2_error,
            ),
            (
                PROTO_DATA.transducer_3,
                PROTO_DATA.error_flags.transducer_3_error,
            ),
        ]
        ...
        THERMOCOUPLE_VALUE_ERROR = [
            (
                PROTO_DATA.thermocouple_1,
                PROTO_DATA.error_flags.thermocouple_1_error,
            ),
            (
                PROTO_DATA.thermocouple_2,
                PROTO_DATA.error_flags.thermocouple_2_error,
            ),
            (
                PROTO_DATA.thermocouple_3,
                PROTO_DATA.error_flags.thermocouple_3_error,
            ),
            (
                PROTO_DATA.thermocouple_4,
                PROTO_DATA.error_flags.thermocouple_4_error,
            ),
        ]
        for i, thermocouple_values in enumerate(THERMOCOUPLE_VALUE_ERROR):
            if (
                not (-1 < thermocouple_values[0] < 34.5)
                or thermocouple_values[1]
            ):
                log_function = slogger.error
            else:
                log_function = slogger.info
...
class GSE_TO_GCS_DATA_2(GSEPacket):
    ...
    def _process_readings(
        self, PROTO_DATA: GSE_TO_GCS_DATA_2_pb.GSE_TO_GCS_DATA_2
    ):
        LOG_TEMP = (
            slogger.info
            if (1 < PROTO_DATA.internal_temp < 60)
            else slogger.error
        )
        LOG_WIND = (
            slogger.info
            if (-0.00001 < PROTO_DATA.wind_speed < 0.00001)
            else slogger.error
        )
        LOG_BOTTLE_1 = (
            slogger.info
            if (15.1 < PROTO_DATA.gas_bottle_weight_1 < 19)
            else slogger.error
        )
        LOG_BOTTLE_2 = (
            slogger.info
            if (15.1 < PROTO_DATA.gas_bottle_weight_2 < 19)
            else slogger.error
        )
        LOG_VAC_1 = (
            slogger.info
            if (2 < PROTO_DATA.analog_voltage_input_1 < 50)
            else slogger.error
        )
        LOG_VAC_2 = (
            slogger.info
            if (-0.00001 < PROTO_DATA.analog_voltage_input_2 < 0.00001)
            else slogger.error
        )
        LOG_CURR_1 = (
            slogger.info
            if (-0.00001 < PROTO_DATA.additional_current_input_1 < 0.00001)
            else slogger.error
        )
        LOG_CURR_2 = (
            slogger.info
            if (-0.00001 < PROTO_DATA.additional_current_input_2 < 0.00001)
            else slogger.error
        )
...
def main(SOCKET_PATH, CREATE_LOGS):
    ...
    while not service_helper.time_to_stop():
        ...
        if len(message) == 1:
            ...
            new_erronous_packet_id = int.from_bytes(message, byteorder="big")
            slogger.error(
        ...
        if packet_id in packet_handlers:
            ...
            try:
                ...
                slogger.error(f"Error processing packet ID {packet_id}: {e}")
