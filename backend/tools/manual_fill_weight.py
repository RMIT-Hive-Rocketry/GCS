...
EXPECTED_FULL_N2O_WEIGHT_KG = (
    EXPECTED_FULL_TOTAL_WEIGHT_KG - EXPECTED_START_TOTAL_WEIGHT_KG
)
...
def process_reading(START_TIME_S: float, mv: Number, csv_path: str):
    ...
    fill_percentage = (
        (weight_kg - EXPECTED_START_TOTAL_WEIGHT_KG)
        / (EXPECTED_FULL_N2O_WEIGHT_KG)
    ) * 100
    ...
    formatted_time = (
        f"{int(cft_minutes):02d}:{cft_whole_seconds:02d}.{cft_milliseconds:03d}"
    )
    ...
    if weight_kg < 0:
        ...
        < weight_kg
        < EXPECTED_FULL_TOTAL_WEIGHT_KG
    ):
        bar = tqdm(total=1, bar_format="{l_bar}{bar} ({percentage:3.0f}%)")
